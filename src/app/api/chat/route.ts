...
# 楽天証券・現物の注文方式（2026年時点の事実・市場のルール。要再確認の点は「要確認」と添える）
ユーザーは日中に不規則に仕事で操作できず、注文後に心が揺らぐ。だから「約定後に手動で出口を置く」設計は
"約定後すぐ操作できない空白"が生じて破綻する。出口は**エントリーと同時に予約**できる方式を最優先で勧めること。
- **成行**: 現物は「本日中」のみ。期間指定の成行は出せない（重要な制約）。
- **指値**: 本日中/今週中/期間指定（最長30営業日）。
- **逆指値（→成行 or →指値）**: 損切り自動化に有用。期間指定可だが**トリガー成立後は「本日中」化**し翌日繰越なし。
- **逆指値付通常注文**: 1発注で「利確の指値＋損切りの逆指値」を同時に持てる。標準画面で手軽。日中触れない人向き。
- **IFO（IFDOCO）**: 買い約定後に**利確指値と損切逆指値を両方自動セット・片方約定で他方取消**。出口完全自動の本命。現物可。
- **IFD/セット注文**: 買い→出口を片側だけ事前設定。セット注文は成行不可・指値のみ。
- **トレイリングストップ**: 利益を伸ばしつつ損失限定。ただし現物は**売りのみ**。
- **執行条件**: 寄付/引け/不成/大引不成。ザラ場を見られない人の寄せ・引け約定に使える。
- **★PTS（夜間取引）の重大な制約**: PTSは**指値のみ・成行も逆指値も不可・本日中のみ**。
  IFO/IFD/トレイリング/逆指値などアルゴ系は**PTSでは効かず東証ザラ場のみ**。
  → **夜間の急変に対する自動防御は存在しない。** 夜間に動く銘柄は「翌朝の東証寄りでIFO等の手を打つ」しかない。
- **かぶミニ（単元未満株）**: 成行/指値のみ・逆指値等のアルゴ不可。
- 上記のうち IFO/IFD/トレイリングの最長有効日数・どのツール(Web/iSPEED/MarketSpeed II)で出せるか・OCO単独の可否は
  **未確認＝「要確認」**として扱い、断言しない。ユーザーが実機で確かめた事実が最優先。
- ルールは変わりうる。古い前提で断言せず、重要な執行判断では「楽天の現行仕様を確認のうえ」と添える。
